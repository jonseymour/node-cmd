...
Dispatcher.prototype._parse = function() {
    ...
    //
    // co-existence with idiomatic-stdio module
    // if --console-to-stderr is specified, console.log()
    // is rebound to process.stderr as if node
    // supported --console-to-stderr as a switch
    //
    // see https://github.com/jonseymour/node-idiomatic-stdio
    // for rationales.
    //

    if (!process._console_selector && options['console-to-stderr']) {
	try {
	    require("idiomatic-stdio").rebind();
	    delete options['console-to-stderr'];
	} catch (x) {
	    console.warn("for --console-to-stderr support, install idiomatic-stdio package");
	}
    }
...
function createDispatcher(config) {
    console.warn("createDispatcher() is deprecated - use ()");
    return use(config);
}
