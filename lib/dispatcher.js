...
function Dispatcher(config)
{
	...
	this.table._select = function(cmd) {
	    ...
	    var handler = ( selector && typeof self.table[selector] === "function") 
	       ? self.table[selector]
	       : undefined;
	    return function() {
		if (handler) {
    ...
    if (!this.table._strategy) {
	this.table._strategy = function(cmd) {

	    if (!(arguments.length == 1 && arguments[0] instanceof command.Command)) {
		cmd = self.table._parse.apply(self.api, arguments);
	    }

	    return (self.table._select.call(self.api, cmd))();
	};
    }

    this.api = this.table._strategy;
